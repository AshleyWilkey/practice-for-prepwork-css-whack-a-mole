...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll(".game-board--mole-head");
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add("game-board--mole-head-hidden");
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener("click", (event) => {
      ...
      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      event.target.classList.add("game-board-mole-head-hidden");
  ...
  window.addEventListener("DOMContentLoaded", () => {
    setInterval(() => {
      const moleHeads = document.querySelectorAll(".game-board--mole-head");
      for (let moleHead of moleHeads) {
        moleHead.classList.toggle("game-board--mole-head-hidden");
      }
    }, 1000);
  });
});
